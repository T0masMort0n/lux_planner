...
---
## Version Anchoring Rule (AI Coding Safety)

All AI coding tasks must be anchored to a specific Git commit hash.

**Requirements:**
- Work must begin from a committed state.
- The commit hash must be provided in the request.
- AI patch instructions are valid only for that commit.
- After applying AI changes, a new commit must be created.
- Prefer working on `ai/<task-name>` branches for AI changes.

This prevents drift, patch misalignment, and outdated file use.

---


## AI Non‑Guessing Enforcement

All agents must follow the Global Non‑Guessing Rule defined in AGENT_SCOPES.  
If required files or context are missing, the agent must stop and request them explicitly.
